...
args['task_name'] = 'Drug'  # change
args['data_name'] = 'Drug'  # change
args['times'] = 10


# selected task, generate select task index, task class, and classification_num
args['select_task_list'] = ['ROA', 'Hematotoxicity', 'neurotoxicity', 'Respiratory', 'TA137_S9', 
                            'TA1538', 'FDAMDD', 'TA104', 'RPMI_8226', 'Hemolytic', 'TA100_S9', 'Mitochondrial', 
                            'reproductive', 'TA102_S9', 'TA1535', 'TA100', 'TA98', 'LOAEL', 'TA97', 
                            'TA97_S9', 'ROA_LD50', 'developmental_toxicity', 'TA104_S9', 'TA1538_S9', 
                            'TA98_S9', 'TA1537', 'Carcinogenicity', 'MRTD', 'TA1535_S9', 'TA102', 
                            'Genotoxicity']  # change
args['select_task_index'] = []
args['classification_num'] = 0
args['regression_num'] = 0
args['all_task_list'] = ['ROA', 'Hematotoxicity', 'neurotoxicity', 'Respiratory', 'TA137_S9', 
                         'TA1538', 'FDAMDD', 'TA104', 'RPMI_8226', 'Hemolytic', 'TA100_S9', 'Mitochondrial', 
                         'reproductive', 'TA102_S9', 'TA1535', 'TA100', 'TA98', 'LOAEL', 'TA97', 
                         'TA97_S9', 'ROA_LD50', 'developmental_toxicity', 'TA104_S9', 'TA1538_S9', 
                         'TA98_S9', 'TA1537', 'Carcinogenicity', 'MRTD', 'TA1535_S9', 'TA102', 
...
for task in args['select_task_list']:
    if task in ['ROA', 'Hematotoxicity', 'neurotoxicity', 'Respiratory', 'TA137_S9', 
                'TA1538', 'FDAMDD', 'TA104', 'RPMI_8226', 'Hemolytic', 'TA100_S9', 'Mitochondrial', 
                'reproductive', 'TA102_S9', 'TA1535', 'TA100', 'TA98', 'TA97', 
                'TA97_S9', 'developmental_toxicity', 'TA104_S9', 'TA1538_S9', 
                'TA98_S9', 'TA1537', 'Carcinogenicity', 'TA1535_S9', 'TA102', 
                'Genotoxicity']:
        args['classification_num'] = args['classification_num'] + 1
    if task in ['ROA_LD50', 'LOAEL', 'MRTD']:
...
print('Classification task:{}, Regression Task:{}'.format(
    args['classification_num'], args['regression_num']))

args['bin_path'] = './data/tox_data_v1/' + args['data_name'] + '/' + args['data_name'] + '.bin'
args['group_path'] = './data/tox_data_v1/' + args['data_name'] + '/' + args['data_name'] + '_group.csv'
...
for time_id in range(args['times']):
    ...
    test_loader = DataLoader(dataset=test_set,
                             batch_size=args['batch_size'],
                             collate_fn=collate_molgraphs)
    pos_weight_np = pos_weight(
        train_set, classification_num=args['classification_num'])
    ...
    optimizer = Adam(model.parameters(), lr=10**-
                     args['lr'], weight_decay=10**-args['weight_decay'])
    stopper = EarlyStopping(
        patience=args['patience'], task_name=args['task_name'], mode=args['mode'])
    model.to(args['device'])

    for epoch in range(args['num_epochs']):
        # Train
        run_a_train_epoch_heterogeneous(
            args, epoch, model, train_loader, loss_criterion_c, loss_criterion_r, optimizer)

        # Validation and early stop
        validation_result = run_an_eval_epoch_heterogeneous(
    ...
    # deal result
    result = train_score + ['training'] + \
        val_score + ['valid'] + test_score + ['test']
