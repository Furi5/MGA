...
args['task_name'] = 'Organ'  # change
args['data_name'] = 'Organ'  # change
args['times'] = 10

df = pd.read_csv('data/tox_data_v1/Organ/Organ.csv')
print(df.columns)
...
for task in args['select_task_list']:
    if task in [col for col in df.columns if col != 'smiles' and col != 'group']:
