...
args['task_name'] = 'Environments'  # change
args['data_name'] = 'Environments'  # change
args['times'] = 10

df = pd.read_csv('data/tox_data_v1/Environments/Environments.csv')
print(df.columns)

# selected task, generate select task index, task class, and classification_num
args['select_task_list'] = [col for col in df.columns if col not in ('smiles', 'group')]  # change
args['select_task_index'] = []
args['classification_num'] = 0
args['regression_num'] = 0
args['all_task_list'] = [col for col in df.columns if col not in ('smiles', 'group')]  # change
...
for task in args['select_task_list']:
    if task in [col for col in df.columns if col not in ('smiles', 'group','BCF','algae_pEC50','crustaceans_pLC50', 'fish_pLC50', 'IBC50', 'LC50DM', 'LC50FM')]:
        args['classification_num'] = args['classification_num'] + 1
    if task in ['BCF','algae_pEC50','crustaceans_pLC50', 'fish_pLC50', 'IBC50', 'LC50DM', 'LC50FM']:
