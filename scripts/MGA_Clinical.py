...
args['task_name'] = 'Clinical'  # change
args['data_name'] = 'Clinical'  # change
args['times'] = 10

df = pd.read_csv('data/tox_data_v1/Clinical/Clinical.csv')
print(df.columns)


# selected task, generate select task index, task class, and classification_num
args['select_task_list'] = [col for col in df.columns if col != 'smiles' and col != 'group']  # change
args['select_task_index'] = []
args['classification_num'] = 0
args['regression_num'] = 0
args['all_task_list'] = [col for col in df.columns if col != 'smiles' and col != 'group'] # change
print(args)
...
for task in args['select_task_list']:
    if task in [col for col in df.columns if col != 'smiles' or col != 'group']:
